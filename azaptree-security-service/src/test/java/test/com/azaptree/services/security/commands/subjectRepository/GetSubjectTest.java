...
@ContextConfiguration(classes = { GetSubjectTest.Config.class })
public class GetSubjectTest extends AzaptreeAbstractTestNGSpringContextTests {
	...
	@Test
	public void test_createSubject_getSubject() {
		...
		ctx.clear();
		ctx.setAttribute(GetSubject.SUBJECT_ID, createdSubject.getEntityId());
		getSubject.execute(ctx);
		final Subject retrievedSubject2 = ctx.getAttribute(GetSubject.SUBJECT);
		Assert.assertEquals(retrievedSubject2.getEntityId(), createdSubject.getEntityId());

	}

	@Test(expectedExceptions = { UnknownSubjectException.class })
	public void test_getSubject_invalidSubjectId() {
		final CommandContext ctx = new CommandContext();
		ctx.setAttribute(GetSubject.SUBJECT_ID, UUID.randomUUID());
		getSubject.execute(ctx);
	}
